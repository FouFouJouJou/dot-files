...
* SSH
#+begin_src emacs-lisp
  ;; TODO make key adding interactive with something like dired or by setting a list variable
  ;; (setq eval-ssh--socket-files '("~/.ssh/dot-files" "~/.ssh/mailer"))
  (defun eval-ssh ()
    (let ((cmd "eval $(ssh-agent -s) && ssh-add ~/.ssh/dot-files ~/.ssh/mailer && echo $SSH_AUTH_SOCK"))
      (progn
